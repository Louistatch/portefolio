/**
 * Projette le contenu du cours FCQ-01 en SQL, et le contrôle au passage.
 *
 *   npx tsx script/generate-fcq-01-sql.ts > supabase/academy_cours_fcq_01.sql
 *
 * Le contenu vit dans shared/fcq-01.ts. Ce script n'en est que la projection : pour corriger
 ...
 */
import { FCQ_01, LECONS_FCQ_01 } from "../shared/fcq-01.js";
...
values (${q(FCQ_01.code)}, ${q(FCQ_01.titre)}, ${q(FCQ_01.description)},
  array[${FCQ_01.outils.map(q).join(",")}]::text[], ${q(FCQ_01.niveau)}, ${LECONS_FCQ_01.length}, 21, true)
...
from sms_courses c where c.code = ${q(FCQ_01.code)}
...
const vides = LECONS_FCQ_01.flatMap(l => l.cellules.filter((c: any) => c.type === "exercise" && !c.explain));
