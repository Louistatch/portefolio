...
function publicationTemplate(name: string | undefined, post: { title: string; slug: string; summary?: string; image_url?: string }) {
  ...
  return baseLayout(`<div class="header"><h1>Nouvelle Publication</h1><p>Louis TATCHIDA vient de publier un nouvel article</p></div><div class="body"><span class="badge">📝 Nouveau contenu</span><p>${g}</p><p>Un nouvel article vient d'être publié :</p><div class="card">${img}<h3>${post.title}</h3>${post.summary ? `<p>${post.summary}</p>` : ""}</div><p style="text-align:center"><a href="${SITE_URL}/blog/${post.slug}" class="cta">Lire l'article</a></p><p>Bonne lecture,<br><strong>Louis TATCHIDA</strong></p></div>`);
}

function campaignTemplate(name: string | undefined, subject: string, content: string) {
  const g = name ? `Bonjour ${name},` : "Bonjour,";
  const html = content.replace(/\n\n/g, "</p><p>").replace(/\n/g, "<br>");
  return baseLayout(`<div class="header"><h1>${subject}</h1><p>Newsletter de Louis TATCHIDA</p></div><div class="body"><p>${g}</p><p>${html}</p><div class="divider"></div><p style="text-align:center"><a href="${SITE_URL}" class="cta">Visiter le site</a></p><p>Cordialement,<br><strong>Louis TATCHIDA</strong></p></div>`);
}
