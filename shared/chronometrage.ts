...
/** La fenêtre de temps qui découle d'un instant de démarrage donné. Fonction pure. */
export function fenetreDepuisDemarrage(demarrage: string, nbQuestions: number): FenetreChrono {
  const duree = dureeEpreuveSecondes(nbQuestions);
  const debut = new Date(demarrage).getTime();
  return { testStartedAt: demarrage, expiresAt: new Date(debut + duree * 1000).toISOString(), durationSeconds: duree };
}
 ...
 */
export function fenetreChronoActive(demarrage: string | null, nbQuestions: number): FenetreChrono | null {
  if (!demarrage) return null;
  const f = fenetreDepuisDemarrage(demarrage, nbQuestions);
